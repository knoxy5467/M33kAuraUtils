...
GroundAuraTracker solves the fundamental limitation of standard WoW buff tracking when dealing with ground-placed spells.
...
2. **Player In-Zone Aura:** While physically standing inside the zone, the player receives an aura (e.g., Spell ID 188370). Stepping out removes this aura immediately (or after a 0.5s grace period).
...
1. **`COMBAT_LOG_EVENT_UNFILTERED`:**
   ...
   - Matches cast spell ID against known ground spells.
...
3. **`OnUpdate` Dispatcher:**
   - Computes exact remaining time: `remaining = max(0, groundExpirationTime - GetTime())`.
   - Updates UI timer text and progress bar smoothly each frame.
